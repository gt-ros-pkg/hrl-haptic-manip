...
%
%   minimize(kappa*(sum(pos(abs(2*mass*qd_1) - tau_max_delta_t)) + sum(pos(abs(2*mass*qd_2) - tau_max_delta_t)) + sum(pos(abs(2*mass*qd_3) - tau_max_delta_t))) + alpha*posture_weight*quad_form(delta_q_des - (q_6 - q_0), eye(7)) + xyz_weight*zeta*quad_form(delta_x_des - J*(q_6 - q_0), eye(3)) + mu*(quad_form(u_0, eye(7)) + quad_form(u_1, eye(7)) + quad_form(u_2, eye(7))) + beta*(sum(pos(n_K_J_all*(q_1 - q_0) - delta_f_max)) + sum(pos(n_K_J_all*(q_2 - q_0) - delta_f_max)) + sum(pos(n_K_J_all*(q_3 - q_0) - delta_f_max))))
%   subject to
...
%   [vars, status] = csolve(params, settings)
% Produced by CVXGEN, 2013-12-03 15:00:45 -0500.
