...
#######################################################
# 
# cRoNA2 Platform Config Script
# 
########################################################
# script that commands robot joint angles into the platform configuration
...
class cRoNA2PlatformConfig(object):
    def __init__(self):
	...
	#self.torso_goal = [-100.]
	#self.torso_goal = [-95.]
	self.torso_goal = [-90.-self.l_shoulder_pan/10.]
	self.head_goal = [90.]
	...
	#self.larm_goal = [105.,0.,self.l_shoulder_pan,80.,self.l_forearm_roll,45.]
	...
	#self.rarm_goal = [105.,0.,self.r_shoulder_pan,80.,self.r_forearm_roll,45.]
...
if __name__ == '__main__':
    rospy.init_node('crona2_platform_config')
    c2pc = cRoNA2PlatformConfig()
    if len(sys.argv) > 1:
    	c2pc.start(float(sys.argv[1]),-float(sys.argv[1]),-float(sys.argv[1]),float(sys.argv[1]),float(sys.argv[2]),float(sys.argv[2]))
    else:
    	c2pc.start()
