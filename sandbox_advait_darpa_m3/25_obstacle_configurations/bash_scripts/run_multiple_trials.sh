...
if [ "$6" = "" ]; then
    echo 'Specify impedance scale'
    exit
fi


find $1 -name "reach_problem_dict*.pkl" -exec python `rospack find sandbox_advait_darpa_m3`/25_obstacle_configurations/bash_scripts/run_single_trial.py {} $2 $3 $4 $5 $6 \;
