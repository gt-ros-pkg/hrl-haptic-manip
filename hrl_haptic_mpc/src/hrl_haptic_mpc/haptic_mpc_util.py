...
def getTaxelArrayDataAsLists(ta_msg):
    ...
    for i in range(0, len(ta_msg.normals_x)):
        normal_vector = np.matrix(
            [ta_msg.normals_x[i], ta_msg.normals_y[i], ta_msg.normals_z[i]]).T
        normals_list.append(normal_vector)
        point_vector = np.matrix(
            [ta_msg.centers_x[i], ta_msg.centers_y[i], ta_msg.centers_z[i]]).T
        locations_list.append(point_vector)
        values_vector = np.matrix(
            [ta_msg.values_x[i], ta_msg.values_y[i], ta_msg.values_z[i]]).T
...
def goalOrientationInQuat(q_h_orient, q_g_orient, max_ang_step):
    ...
    interp_q_goal = tr.tft.quaternion_slerp(
        q_h_orient, q_g_orient, step_fraction)
...
def initialiseOptParser(p):
    ...
    p.add_option('--controller', '-c', action='store', dest='controller',
                 help='controller type: static, dynamic')
    ...
    p.add_option('--task', '-t', action='store', dest='task',
                 help='task name: lic, planner, etc')
...
def getValidInput(p):
    ...
    valid_robots = ['cody', 'cody5dof', 'pr2', 'sim3', 'sim3_nolim', 'sim3_with_hand',
                    'simcody', 'crona', 'darpa', 'sim_equal_links_1', 'kreacher', 'darci', 'darci_sim']
    valid_sensors = ['meka', 'fabric', 'hil', 'none', 'pps', 'sim']
    valid_controllers = ['static', 'dynamic']
    ...
    if opt.controller and opt.controller not in valid_controllers:
        p.error("Must specify a valid controller type: -s %s" %
                str(valid_controllers))
    if opt.robot and opt.sensor and opt.robot == 'pr2' and (opt.sensor == 'meka' or opt.sensor == 'hil'):
        p.error(
            "Invalid sensor for the PR2 [meka, hil]. Valid options: -s [fabric, pps, none]")
    return opt
