...
// NOW ALSO HAS POSTURE CONTROL AND DOESN'T USE F_DELTA_MAX. Function for posture is runController_posture


// user defined functions for boost python to interpret
// Has target delta_q_desired (called delta_q_des). 
boost::python::list runController_posture(double alpha,
				  double beta,
				  double kappa,
				  double mu,
...
BOOST_PYTHON_MODULE(cvxgen_controller_posture)
{
  using namespace boost::python;
  def("greet", greet);
  def("return_stuff", return_stuff);
  def("runController_posture", runController_posture);
}
