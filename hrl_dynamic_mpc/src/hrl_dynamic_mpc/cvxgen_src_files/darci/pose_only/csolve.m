...
%
%   minimize(kappa*(sum(pos(abs(2*mass*qd_1) - tau_max_delta_t)) + sum(pos(abs(2*mass*qd_2) - tau_max_delta_t)) + sum(pos(abs(2*mass*qd_3) - tau_max_delta_t))) + alpha*quad_form(delta_x_d - J*(q_6 - q_0), eye(3)) + mu*(quad_form(u_0, eye(7)) + quad_form(u_1, eye(7)) + quad_form(u_2, eye(7))) + beta*(sum(pos(n_K_J_all*(q_1 - q_0) - delta_f_max)) + sum(pos(n_K_J_all*(q_2 - q_0) - delta_f_max)) + sum(pos(n_K_J_all*(q_3 - q_0) - delta_f_max))) + zeta*(sum(pos(n_K_J_all*(q_1 - q_0) - delta_rate_f_max)) + sum(pos(n_K_J_all*(q_2 - q_1) - delta_rate_f_max)) + sum(pos(n_K_J_all*(q_3 - q_2) - delta_rate_f_max))) + zeta*(sum(pos( - n_K_J_all*(q_1 - q_0) - delta_rate_f_max)) + sum(pos( - n_K_J_all*(q_2 - q_1) - delta_rate_f_max)) + sum(pos( - n_K_J_all*(q_3 - q_2) - delta_rate_f_max))))
%   subject to
...
%   [vars, status] = csolve(params, settings)
% Produced by CVXGEN, 2014-01-15 15:36:01 -0500.
