...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  this_var_errors = 0;
  xm = mxGetField(prhs[0], 0, "delta_rate_f_max");
  if (xm == NULL) {
    printf("could not find params.delta_rate_f_max.\n");
  } else {
    if (!((mxGetM(xm) == 10) && (mxGetN(xm) == 1))) {
      printf("delta_rate_f_max must be size (10,1), not (%d,%d).\n", mxGetM(xm), mxGetN(xm));
      this_var_errors++;
    }
    if (mxIsComplex(xm)) {
      printf("parameter delta_rate_f_max must be real.\n");
      this_var_errors++;
    }
    if (!mxIsClass(xm, "double")) {
      printf("parameter delta_rate_f_max must be a full matrix of doubles.\n");
      this_var_errors++;
    }
    if (mxIsSparse(xm)) {
      printf("parameter delta_rate_f_max must be a full matrix.\n");
      this_var_errors++;
    }
    if (this_var_errors == 0) {
      dest = params.delta_rate_f_max;
      src = mxGetPr(xm);
      for (i = 0; i < 10; i++)
        *dest++ = *src++;
      valid_vars++;
  ...
  if (prepare_for_c) {
    ...
    for (i = 0; i < 10; i++)
      printf("  params.delta_rate_f_max[%d] = %.6g;\n", i, params.delta_rate_f_max[i]);
    for (i = 0; i < 49; i++)
