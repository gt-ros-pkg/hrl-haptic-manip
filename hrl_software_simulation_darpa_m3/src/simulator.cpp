...
int main(int argc, char **argv)
{
	...
	/* this section may no longer be necessary though because
	   we are now synchronizing the mpc controller and simulation 
	   at least in some branches of the git code. - marc Sept 2012 */
	    ...
	    simulator.update_proximity_simulation();
            ...
            skin_step = 0;
	    //std::cout << "calling service to wait for optimization \n";
	    hrl_haptic_manipulation_in_clutter_srvs::GetJep opt_srv;
	    bool test_call;
	    if  (simulator.new_data_serv.call(opt_srv))
	      {
		simulator.add_delta_jep(opt_srv.response.delta_jep);
		//std::cout << "got delta_jep .." << std::endl;
	      }
	    // else
	    //   {
	    // 	//std::cout << "no change in jep ..." << std::endl;
	    //   }
        }
