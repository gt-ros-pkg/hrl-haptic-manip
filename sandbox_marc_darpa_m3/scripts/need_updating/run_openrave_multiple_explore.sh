...
do
	...
	do
	    ...
	    cd $D
	    if [ "$4" == 1 ]; then   
		if [ -f ./openrave_result_all.pkl ]
		then
		    echo "openrave_result_all.pkl already exists, skipping folder"
		else
		...
		if [ -f ./openrave_result_ignore_moveable.pkl ]
		then
		    echo "openrave_result_ignore_moveable.pkl already exists, skipping folder"
		else
