...
if __name__ == '__main__':
    rospy.init_node('crona2_lower_down')
    c2ld = cRoNA2LowerDown()
    if len(sys.argv) > 1:
    	c2ld.start(float(sys.argv[1]),-float(sys.argv[1]),-float(sys.argv[1]),float(sys.argv[1]),float(sys.argv[2]),float(sys.argv[2]))
    else:
    	c2ld.start()
