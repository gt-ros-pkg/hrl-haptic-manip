...
class LoggerAndMonitor():
    def __init__(self, skin_client, robot, log_taxel_array=False):
        self.sc = skin_client
        self.robot = robot
        self.pause_tm = False
        ...
        # rate that jep is being sent by the controller (only used for simulation synch)
        self.jep_send_rate = cb.CircularBuffer(self.hist_size, ())
        ...
        self.log_taxel_array = log_taxel_array
        ...
        if log_taxel_array == True:
            rospy.Subscriber("/skin/taxel_array", TaxelArray, self.taxel_cb)
        ...
        rospy.Subscriber('/controller_time_step', Float64, self.jep_rate_cb)
    ...
    def taxel_cb(self, msg):
        if self.taxel_hist == None:
            self.taxel_hist = np.array([0]*len(msg.forces_x))
        ind_arr = np.any([np.array(msg.forces_x) != 0.0, np.array(msg.forces_y) != 0.0, np.array(msg.forces_z) != 0.0], axis=0)
        temp_hist = np.where(ind_arr, 1, 0)
        self.taxel_hist = self.taxel_hist + temp_hist
    ...
    def log_and_monitor_info(self, req):
        rospy.loginfo('started logging due to request')
        tp = req.torso_pose
        p = tp.position
    ...
    def update_common_log(self):
        ...
        self.update_mean_motion(self.ee_gaussian_mn_buf_50, step=1000)
    
    def log_to_dict(self, controller):
        # common to all the controllers.
        d = {}
        d['jep_send_rate'] = self.jep_send_rate.to_list()
        ...
        if self.log_taxel_array:
            d['taxel_hist'] = self.taxel_hist
    ...
    def jep_rate_cb(self, msg):
        self.jep_send_rate.append(msg.data)

    def clear_log(self, controller):
        # common to all controllers.
        self.jep_send_rate.clear()
...
if __name__ == '__main__':
    ...
    p.add_option('--arm_to_use', action='store', dest='arm',
                 ...
                 default=None)
    p.add_option('--log_taxel_msg', action='store_true', 
                 default=False, dest='log_taxel_msg',
                 help='log the data, but disable sending a stop command.')
    ...
    if opt.log_taxel_msg == True:
        lm = LoggerAndMonitor(scl, robot, opt.log_taxel_msg)
    else:
