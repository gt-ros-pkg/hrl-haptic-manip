...
# 
########################################################
# uses joint control to lift ragdoll after ragdoll is scooped
...
class cRoNA2Lift(object):
	...
	previous_set_pose = self.set_pose
        self.send_goal(self.set_pose,10,10)
	print "Part 1"
	self.torso_goal = [0.]
        self.head_goal = [0.]
	self.larm_goal = [45.,0.,self.l_shoulder_pan,85.,self.l_forearm_roll,45.]
	#self.larm_goal = [45.,0.,self.l_shoulder_pan,85.,self.l_forearm_roll,45.]
        self.rarm_goal = [45.,0.,self.r_shoulder_pan,85.,self.r_forearm_roll,45.]
        #self.rarm_goal = [45.,0.,self.r_shoulder_pan,85.,self.r_forearm_roll,45.]
	self.set_pose = self.base_ja+self.torso_goal+self.head_goal+self.larm_goal+self.rarm_goal
        self.send_goal(self.set_pose,20,10,previous_set_pose)
	print "Part 2"
...
if __name__ == '__main__':
    ...
    c2l = cRoNA2Lift()
    if len(sys.argv) > 1:
        c2l.start(float(sys.argv[1]),-float(sys.argv[1]),-float(sys.argv[1]),float(sys.argv[1]))
    else:
        c2l.start()
