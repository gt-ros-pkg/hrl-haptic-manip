...
class ComputeResultStatistics():
    ...
    def get_impact_and_jamming_data(self, full_path):
        ...
        if full_path == 'end':
            self.contact_d = {}
            self.contact_d['contact_vel'] = self.contact_vel
            self.contact_d['contact_force'] = self.contact_force
            self.contact_d['contact_locs'] = self.contact_locs
            self.contact_d['contact_ind'] = self.contact_ind
            # ut.save_pickle(d, self.root_path + '/stats_on_impact_velocity.pkl')

            self.jamming_d = {}
            self.jamming_d['total_num_cont_jamming'] = self.total_num_cont_jamming
            self.jamming_d['jamming_force'] = self.jamming_forces
            self.jamming_d['jamming_vel'] = self.jamming_vel
            self.jamming_d['jamming_normals'] = self.jamming_normals
            self.jamming_d['jamming_ind'] = self.jamming_ind
            # ut.save_pickle(d, self.root_path + '/stats_on_jamming.pkl')
            ...
            for i in xrange(len(data['q_list'])):
                if data['num_contacts_at_time_instant_list'][i] > 0:
                    ...
                    for j in xrange(int(data['num_contacts_at_time_instant_list'][i])):
                        ...
                        self.contact_ind.append(counter)
                        counter = counter + 1
                ...
                if data['num_contacts_at_time_instant_list'][i] > 1:
                    ...
                    for j in xrange(int(data['num_contacts_at_time_instant_list'][i])):
                        ...
                        ind_buf.append(counter_jamming)
                        vel_buf.append(vel)
