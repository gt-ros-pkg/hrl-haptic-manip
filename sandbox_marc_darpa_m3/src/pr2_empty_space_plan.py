...
def main():
    global traj, solutions, robot

    env=Environment()
    env.SetViewer('qtcoin')
    env.Load('./pr2test1.env.xml')
    ...
    torso_lift_link = robot.GetLink('torso_lift_link')

    # print "torso_lift_link transform is :\n", robot.GetLinks()[16].GetTransform()
    # print "base_link transform is :\n", robot.GetLinks()[1].GetTransform()

    with env:
        T_torso_world = torso_lift_link.GetTransform()

        T_des_torso = array([[ 1. , 0. , 0. ,  0.8 ],
                             [ 0. , 1. , 0. ,  -0.6],
                             [ 0. , 0. , 1. ,  0.1],
                             [ 0. , 0. , 0. ,  1.]])

        # T_des_torso = array([[ 0. , 1. , 0. ,  0.5 ],
        #                      [ -1. , 0. , 0. ,  -0.4],
        #                      [ 0. , 0. , 1. ,  0.3],
        #                      [ 0. , 0. , 0. ,  1.]])

        R_offset = array([[0., 0., 1.],
                          [0., 1., 0.],
                          [-1., 0., 0.]])

        T_des_torso[0:3,0:3] = dot(T_des_torso[0:3,0:3], R_offset)

        #T = manip.GetEndEffectorTransform() # get end effector

        T = dot(T_torso_world, T_des_torso)

        print "T is :\n", T

        # check if ik solutions exist
        sol0=manip.FindIKSolution(T,IkFilterOptions.CheckEnvCollisions)
        assert(sol0 is not None)

    traj = basemanip.MoveToHandPosition(matrices=[T], execute=True, outputtrajobj=True, seedik=True)  #seedik almost doubles time to solve
    #traj = basemanip.MoveToHandPosition(matrices=[T], execute=True, outputtrajobj=True)  #seedik almost doubles time to solve
    planningutils.RetimeAffineTrajectory(traj,maxvelocities=0.1*ones(7),maxaccelerations=5*ones(7))

    end = time.time()



    print "diff in time is ", end-start
    #basemanip.MoveToHandPosition(matrices=[T])
    robot.WaitForController(0)
    ...
    # env.SetViewer('qtcoin')
    # robot = env.GetRobots()[0]
    # goal = array([[1., 0., 0., 0.4],[0., 1., 0., 0.], [0., 0., 1., 0.3],[0., 0., 0., 1.]])

    # basemanip = interfaces.BaseManipulation(robot)
    # with robot:
    #     traj = basemanip.MoveToHandPosition(matrices=[goal],execute=False,outputtrajobj=True)
    ...
    # #make a pr2 arm object for later to get joint angles
    # q = [0.]*7
    # robot.SetDOFValues(q, range(len(q)))
    # #manipprob = interfaces.BaseManipulation(robot, 'BiRRT')   # or BasicRRT
    # #manipprob = interfaces.BaseManipulation(robot, 'BiRRT') 

    # #goal = [0.8, 0.0, 0.0, 0., 0., 0.]  #this needs to be in homogeneous coordinates
    # goal = array([[1., 0., 0., 0.4],[0., 1., 0., 0.], [0., 0., 1., 0.3],[0., 0., 0., 1.]])

    # print "goal is :\n", goal
    # #START HERE TO PULL STUFF OUT OF "with env" below
    # #START HERE TO PULL STUFF OUT OF "with env" below
    #     #START HERE TO PULL STUFF OUT OF "with env" below

    # solutions = ikmodel.manip.FindIKSolution(goal,IkFilterOptions.CheckEnvCollisions)

    # #sol = ikmodel.manip.FindIKSolution(goal)
    ...
    # traj.Init(spec)
    # traj.Insert(traj.GetNumWaypoints(),poseFromMatrix(goal))
