    ...
    for pp = 1:1:length(delta_t_vec)
        for dd = 1:1:length(beta_vec)
            for kk = 1:1:length(waypoint_vec);
                ...
                x_g = [0.165*cos(pi/4), 0.165*sin(pi/4)]';
                ...
                params.q_min = deg2rad(-80)*ones(1,1);
                params.q_max = deg2rad(80)*ones(1,1);
                ...
                while i < 16. && flag_success == false
                    ...
                    params.q_0 = q_0';
                    params.B = eye(1) %would normally at least vary according to contacts
                    params.J = J;
                    ...
                    [vars, status] = csolve_single_link_QS_horiz_10(params); %csolve(params);
                ...
                path = strcat('./variable_control_horizon_tests/single_link/QS_horizon_',num2str(horizon), ...
