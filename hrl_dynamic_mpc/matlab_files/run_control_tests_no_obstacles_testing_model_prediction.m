...
if exist('model_prediction_tests_q_init_num_1_goal_num_1.mat') == 0
    q_init_vec = [rand(num_q_init,1)*(q_max(1) - q_min(1)) + q_min(1), rand(num_q_init,1)*(q_max(2) - q_min(2)) + q_min(2), rand(num_q_init,1)*(q_max(3) - q_min(3)) + q_min(3)];
    goal_vec = [rand(num_goals,1)*(x_max-x_min)+x_min, rand(num_goals,1)*(y_max-y_min)+y_min];
else
...
for m = 1:1:length(q_init_vec)
    for tt = 1:1:length(goal_vec)
        file_name = strcat('./model_prediction_tests_q_init_num_', num2str(m), '_goal_num_', num2str(tt),'.mat');
