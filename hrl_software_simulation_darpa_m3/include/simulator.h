...
#include "hrl_haptic_manipulation_in_clutter_srvs/GetJep.h"
...
class Simulator{
	...
	void add_delta_jep(const hrl_msgs::FloatArrayBare msg);
	...
	dSimpleSpace space;
	dWorld world;
	static const dReal timestep=0.0005;  //was 0.0005
	double cur_time;
	ros::ServiceClient new_data_serv;
    ...
    protected:
        ...
        hrl_msgs::FloatArrayBare jt_torques;
    ...
    nh_(nh)
{
    ...
    torque_pub = nh_.advertise<hrl_msgs::FloatArrayBare>("/sim_arm/torques", 100);
    ...
    new_data_serv = nh_.serviceClient<hrl_haptic_manipulation_in_clutter_srvs::GetJep>("new_data_ready");
    ...
    //dWorldSetERP(world.id(), 1);
    //dWorldSetCFM(world.id(),  0);
}
    ...
    if (numc > 0)
    {
        ...
        {// here contact is between two objects.
            for (int i=0; i<numc; i++)
            {
                ...
                /* contact[i].surface.soft_cfm = 0.002164; */
                /* contact[i].surface.soft_erp = 0.05193; */
        ...
        { // here contact is between a link of the arm and an object.
            ...
            {
                contact[i].surface.mode = dContactSoftCFM | dContactApprox1 | dContactSoftERP;
                contact[i].surface.mu = 0.4; //check original colossus if this was 0.4
...
void Simulator::create_robot()
{
    ...
    for (int ii = 0; ii < num_jts; ii++)
    {
	    ...
	    //dJointSetHingeParam(manip_rev_jts[ii].id(), dParamCFM, 0.0);
...
void Simulator::add_delta_jep(const hrl_msgs::FloatArrayBare msg)
{
  for (int ii = 0; ii < num_jts ; ii++)
    {
      jep[ii] = jep[ii]+msg.data[ii];
    }
}
