...
num_poss_contacts = 12

class CvxGenData():
    def __init__(self, robot_type, use_orientation=False, options=None):
        ...
        self.nom_stiffness = 48000.
        self.nom_damping = 1.0
        self.max_force = 0.
        self.q_s = []
        self.qd_s = []
        self.max_forces = []
        ...
        #self.alpha = 22.67 #20.2 # importance of predicted distance between end-effector and goal at the horizon (terminal cost)
        self.alpha = 10. #20.2 # importance of predicted distance between end-effector and goal at the horizon (terminal cost)
        self.beta = 52.18 #341. # importance of reducing each predicted contact force below user specified force threshold 
        self.zeta = 31.57 #0.01 # importance of keeping the predicted change in force at each step below specified rate (see self.force_rate)
        self.mu = 1.067 #6.56 # importance of keeping contol effort low (keep change in equilibrium angle low - proportional to change in torque for quasi-static)
        self.gamma = 100000. #1000000000000000000. #1000.
        self.waypoint_mag = 0.04

        # cost function params
        self.force_rate = 0.2051 #0.0645 # no penalty for predicted magnitude of change of contact force being below this value.

        # constraint params
        self.u_max_nom = 0.262 #0.543 # (radians) maximum allowed magnitude of the change in the equilibrium angle 
        self.delta_t_impulse = 0.207 #0.228 # (seconds) impulse collision model - time over which impact joint torques occur (smaller value means arm moves more slowly) #0.04 #0.05 
        ...
        if robot_type == "sim":
            self.q_min = np.radians([-151, -64, -1])
            self.q_max = np.radians([152, 164, 161])
        ...
        if options != None:
            if options.f_thresh != None:
                self.f_thresh = float(options.f_thresh)
                print "f_thresh is now :", self.f_thresh
            if options.tuning == True:
                self.set_params_from_file(options.impulse_slope)

        s = rospy.Service('new_data_ready', GetJep, self.waiting_for_data_from_sim)
        self.send_to_cvxgen = rospy.ServiceProxy("solve_qp_problem", MpcFormatted, persistent=True)
    ...
    def saveDataToDisk(self, success, final_state):
        ...
        ut.save_pickle({'not_converged':self.converged_counter}, './converged_log.pkl')
        overall_dict = {}
  ...
  # Called when data should be dumped to disk (infrequent)
    ...
    def populate_dyn_mpc_msg(self, delta_x_g, J_h, q, q_dot, q_des_cur, Kp, Kd,
        ...
        #with self.lock:
            ...
            #print "shape is :", np.matrix([[delta_x_g[1]], [-delta_x_g[0]]])).shape
            vel_normal = np.matrix([[delta_x_g[1,0], -delta_x_g[0,0]]]).reshape(1,2)
            vel_normal = vel_normal/np.linalg.norm(vel_normal)
            #J_h_augmented = np.vstack((J_h, np.zeros((1, num_joints))))
            vel_norm_J = vel_normal*J_h #goal_cross_mat*J_h_augmented
            # print "J_h is :", J_h
            # print "vel_norm_J is :\n", vel_norm_J
            # print "vel_normal is :\n", vel_normal
            ...
            if False:
                ...
                u_max = [self.u_max_nom]*len(q_max) # should be 0.04
                self.out_msg.u_max = u_max
            ...
            if False:
                ...
                max_impulse_test = []
                tau_impulse_ls = []
                delta_t_impulse = self.delta_t_impulse #0.04 #0.05 # .07 #0.02-3 worked pretty well (10N or so maximums)cally

                #new approach that Charlie wants
                offset = 0.02
                # for kk in xrange(len(q)):
                #     vec = self.jt_pos[kk+1] - self.jt_pos[kk]
                #     max_pos = vec/np.linalg.norm(vec)*offset+self.jt_pos[kk]
                #     f_dir = np.cross(([0, 0, 1], max_pos-self.jt_pos[kk]).A1) 
                #     tau_impulse_max = np.linalg.norm(np.cross((max_pos-self.jt_pos[kk]).A1, f_dir*self.f_thresh))
                #     max_impulse_test.append(tau_impulse_max*delta_t_impulse)

                #this should do the same thing as the loop above
                max_impulse_test = [self.f_thresh*offset*delta_t_impulse]*len(q)
            ...
            all_J_T_Kd_J = np.matrix(np.zeros((num_joints, num_joints)))
            #print "Kc_l is :\n", Kc_l
            ...
            for ii in xrange(len(Jc_l)): 
                ...
                all_J_T_Kd_J = all_J_T_Kd_J + Jc_l[ii].T*Kd_l[ii]*Jc_l[ii]

                #print "for i = ", ii, " all_J_T_K_J is \n:", Jc_l[ii].T*Kc_l[ii]*Jc_l[ii] 
                
                if values_n[ii,0] > 0.01: 
                    diff = self.f_thresh - values_n[ii,0] - 1. 
                    buff = n_l[ii].T*Kc_l[ii]*Jc_l[ii] 
                    buff2 = n_l[ii].T*Kd_l[ii]*Jc_l[ii] 

                    if values_n[ii,0] > self.max_force: 
                        self.max_force = values_n[ii,0] 

                    # if abs(diff) > 5:
                    #     diff = -(f_thresh - values_n[ii,0])**2
                    ...
                    buff2_list = (scaling_factor*buff2).A1.tolist()
                    n_K_J_all_buff.append(buff_list)
                    n_Kd_J_all_buff.append(buff2_list)
            ...
            # this was a dirty dirty hack that may have been causing problems, new solution
            # addresses this using the first option listed above
            ######################################################################
            ...
            cor_mat = Coriolis.A  #.A gives the array from the np.matrix

            #these are the continuous time state space equations at the current time step
            A = np.matrix(np.vstack((np.hstack((inv_mass*(-Kd-cor_mat), inv_mass*(-Kp-all_J_T_K_J))),
                          np.hstack((np.eye(num_joints), np.zeros((num_joints, num_joints)))))))
            
            B = np.matrix(np.vstack((
                          np.hstack((inv_mass*Kp, inv_mass, inv_mass*all_J_T_K_J)),
            ...
            self.out_msg.B_t1 = B_t1.flatten('F').A1.tolist()
            self.out_msg.B_t2 = B_t2.flatten('F').A1.tolist()
            self.out_msg.B_t3 = B_t3.flatten('F').A1.tolist()
            self.out_msg.B_b1 = B_b1.flatten('F').A1.tolist()
            self.out_msg.B_b2 = B_b2.flatten('F').A1.tolist()
            self.out_msg.B_b3 = B_b3.flatten('F').A1.tolist()

            #print "same flattened :\n", all_J_T_K_J.flatten('F').A1.tolist()

            J_T_K_J_eig , _ = np.linalg.eig(all_J_T_K_J)
            
            #this term can probably disappear now ############
            self.out_msg.state_scaling = 1.
            #this term can probably disappear now ############
            
            self.out_msg.all_J_T_K_J = all_J_T_K_J.flatten('F').A1.tolist()
            #self.out_msg.all_J_T_Kd_J = all_J_T_Kd_J.flatten('F').A1.tolist()

            #this is a check for any contact at all before populating matrices
            if delta_rate_force_max_buff == []:
                delta_rate_force_max = np.ones((n,1))*0.01
            else:
                delta_rate_force_max = np.vstack((np.array([delta_rate_force_max_buff]).T, 
                                                  0.01*np.ones((n-len(delta_rate_force_max_buff),1))))

            if delta_force_max_buff == []:
                delta_force_max = np.ones((n,1))*1
            else:
                delta_force_max = np.vstack((np.array([delta_force_max_buff]).T, 
                                             np.ones((n-len(delta_force_max_buff),1))*1.))
            #print "delta_force_max after augmenting: \n", delta_force_max

            #print "n_K_J_all :\n", np.array(n_K_J_all_buff)
            if n_K_J_all_buff == []:
                n_K_J_all = np.zeros((num_poss_contacts, len(q)))
                n_Kd_J_all = np.zeros((num_poss_contacts, len(q)))
            else:
                n_K_J_all = np.array(n_K_J_all_buff)
                n_K_J_all = np.vstack((n_K_J_all, np.zeros((num_poss_contacts-n_K_J_all.shape[0], len(q)))))
                n_Kd_J_all = np.array(n_Kd_J_all_buff)
                n_Kd_J_all = np.vstack((n_Kd_J_all, np.zeros((num_poss_contacts-n_Kd_J_all.shape[0], len(q)))))
            ...
            self.out_msg.n_Kd_J_all = n_K_J_all.flatten('F').tolist()
    ...
    def delta_force_bounds(self, values_n, f_thresh):
        ...
        for i in xrange(len(values_n)):
            if values_n[i,0] > 0.01:
                ...
                #if diff >= 0.:
                #    delta_force_max_buff.append(diff/self.nom_stiffness)
                #    delta_force_min_buff.append(-0.5/self.nom_stiffness)
                #    #print "delta_force_max_buff :", delta_force_max_buff
                #else:
                #    delta_force_max_buff.append(-0.5/self.nom_stiffness)
                #    delta_force_min_buff.append(-2.0/self.nom_stiffness)
                #    #print "delta_force_max_buff :", delta_force_max_buff
    ...
    def get_contact_gain_in_matrix_form(self, n_l, k_default):
        Kc_l = []
        for n_ci in n_l:
    ...
    def update_data(self):
        ...
        with self.lock:
            ...
            Kc_l = self.get_contact_gain_in_matrix_form(n_l, self.nom_stiffness)
            Kd_l = self.get_contact_gain_in_matrix_form(n_l, self.nom_damping)
            ...
            Coriolis = np.matrix(c_mat(self.params, self.q, self.q_dot))
            #Coriolis = np.matrix(c(self.params, self.q, self.q_dot)).reshape(self.n_jts, 1)
    ...
    def waiting_for_data_from_sim(self, req):
        ...
        if np.linalg.norm(resp.delta_jep.data) == 0: 
        ...
        if True:
            ...
            B_b1 = np.matrix(self.out_msg.B_b1).reshape(self.n_jts,self.n_jts,order='F')
            B_b2 = np.matrix(self.out_msg.B_b2).reshape(self.n_jts,self.n_jts,order='F')
            B_b3 = np.matrix(self.out_msg.B_b3).reshape(self.n_jts,self.n_jts,order='F')
            B_t1 = np.matrix(self.out_msg.B_t1).reshape(self.n_jts,self.n_jts,order='F')
            B_t2 = np.matrix(self.out_msg.B_t2).reshape(self.n_jts,self.n_jts,order='F')
            B_t3 = np.matrix(self.out_msg.B_t3).reshape(self.n_jts,self.n_jts,order='F')
    ...
    def set_params_from_file(self, impulse_slope):
        print "setting params from x pickle"
        self.lock.acquire()
        d = ut.load_pickle('./x.pkl')
        x = d['x']

        self.alpha = x[0] # alpha
        self.beta = x[1] # weight on force threshold
        self.zeta = x[2] # weight on force change rate
        self.mu = x[3] # weight on control effort
        if impulse_slope == True:
            self.delta_t_impulse = x[4]*(self.f_thresh)*(-0.5/20.0) + 0.825
            print "delta_t_impulse is :", self.delta_t_impulse
            print "for thresh :", self.f_thresh
        else:
            self.delta_t_impulse = x[4] # param for impact time in impulse model
        self.u_max_nom = x[5] # param for max allowable change in u
        self.force_rate = x[6] # param for allowable change in force per time
        self.waypoint_mag = x[7] # size of waypoint slewing
        #self.gamma = x[7] # weight for straight line motion
...
if __name__ == '__main__':
    ...
    p.add_option('--tuning_params', action='store_true', dest='tuning',
                 default=False, help='should I be setting params from file?')

    p.add_option('--impulse_slope', action='store_true', dest='impulse_slope',
                 default=False, help='should I use the t_impulse param as a slope?')

    p.add_option('--f_thresh', action='store', dest='f_thresh',
                 default=None, help='specifiy f_thresh')

    opt, args = p.parse_args()

    gen_data = CvxGenData('sim', False, options=opt)
