# code downloaded from http://prideout.net/blog/?p=44#subdivision on July 7, 2011
# Written by Philip Rideout
# stated it is in the public domain
# very minor modification by Charlie Kemp
...
def sphere_vertices(num_subdivisions):
    verts, faces = icosahedron()
    # improve precision
    verts = [Vector3(*v).normalized()[:] for v in verts]
    for x in xrange(num_subdivisions):
        verts, faces = subdivide(verts, faces)
    unique_verts = set(verts)
    return np.matrix([v for v in unique_verts])
