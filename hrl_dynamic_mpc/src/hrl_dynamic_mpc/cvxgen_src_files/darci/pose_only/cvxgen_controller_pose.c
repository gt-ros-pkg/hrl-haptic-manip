...
boost::python::list runController_pose(double alpha,
				  double beta,
				  double kappa,
				  double zeta,
				  double mu,
				  boost::python::list delta_x_d,
				  boost::python::list J,
				  ...
				  boost::python::list delta_rate_f_max,
				  boost::python::list n_K_J_all)
{
  ...
  int ii;
  for (ii=0; ii < boost::python::len(delta_x_d); ii++)
    {
      //printf("delta_x_d %d is %0.5f \n", ii, delta_x_d[ii]);
      params.delta_x_d[ii] = boost::python::extract<double>(delta_x_d[ii]);
    }
  ...
  for (ii=0; ii < boost::python::len(delta_f_max); ii++)
    {
      ...
      params.delta_rate_f_max[ii] = boost::python::extract<double>(delta_rate_f_max[ii]);
    }
...
BOOST_PYTHON_MODULE(cvxgen_controller_pose)
{
  using namespace boost::python;
  def("greet", greet);
  def("return_stuff", return_stuff);
  def("runController_pose", runController_pose);
}
