...
for i = 1:1:50
    ...
    % x_g = [0.383, 0.475]';
    x_g = random('unif', 0, 1, 2, 1);
    x_g(1) = x_g(1)*(0.5-(-0.2))+(-0.2);
    x_g(2) = x_g(2)*(0.6-(0.4))+(0.4);
    obst_pos = random('unif', 0, 1, 20, 3)
    obst_pos(:,3) = 0;
    obst_pos(:,1) = obst_pos(:,1)*(0.5-(-0.2))+(-0.2);
    obst_pos(:,2) = obst_pos(:,2)*(0.6-(0.15))+(0.15);
    ...
    %copy from here down
    delta_t_impulse = .08; %0.05; %0.03;  % started with 0.07 update this?
    Kp = diag([30, 20, 15]);
    Kd = diag([20, 15, 8]);
    delta_t = 0.01;
    q_0 = deg2rad([-90, 105, 120]);
    q_des_cur_0 = q_0;
    qd_0 = [0, 0, 0];
    ...
    contact_stiffness = 10000;  %5000 48000
    contact_damping = 10;    %100 1
    params.Kp = Kp;
    params.Kd = Kd;
    params.delta_rate_f_max = 10.*ones(num_max_contacts,1);
    params.alpha = 275;
    params.mu = 34.8;
    params.zeta = 0.01;
    params.beta = 229.;
    %params.gamma = 10.;
    params.u_max = [10., 10., 10.]'; %update this
    ...
    flag_success = false;
    i  = -0.01;
    while i < 10.0 && flag_force == false && flag_success == false
        %for i = 0:0.01:0.01
        ...
        %delta_x_d = x_g - x_cur;
        if norm(x_g-x_cur) > 0.04
            delta_x_d = (x_g - x_cur)/norm(x_g-x_cur)*0.04;
        else
            delta_x_d = x_g - x_cur;
        end
        ...
        if length(max_forces)>1
            if max_forces(end-1) > 0.01
                if (max_forces(end) - max_forces(end-1)) < 0
                    flag_force = true;
                    max_force_08 = [max_force_08, max(max_forces)]
                end
...
%         plot(x_g(1), x_g(2), 'g*');
    ...
    delta_t_impulse = 5.; %0.05; %0.03;  % started with 0.07 update this?
    Kp = diag([30, 20, 15]);
    Kd = diag([20, 15, 8]);
    delta_t = 0.01;
    q_0 = deg2rad([-90, 105, 120]);
    ...
    while i < 10.0 && flag_force == false && flag_success == false
        ...
        if length(max_forces)>1
            if max_forces(end-1) > 0.01
                if (max_forces(end) - max_forces(end-1)) < 0
                    flag_force = true;
                    max_force_5 = [max_force_5, max(max_forces)]
                end
...
%         plot(x_g(1), x_g(2), 'g*');
    ...
    save('./intermed_impulse_results', 'max_force_08', 'max_force_5')
end
