...
def return_lists_50_percent_each():
    dyn_skin_nm_list = [
                         ...
                         #'dyn_vs_qs_comparison/dyn_high_force_high_clutter_20130624_v2_large_delta_t_impulse_0.02',
                         #'dyn_vs_qs_comparison/dyn_high_force_high_clutter_simple_large_delta_t_impulse_0.02',
                         'dyn_vs_qs_comparison/dyn_high_force_high_clutter_new_delta_t_impulse_sim_anneal_search_set2_mu_15',

                         #'dyn_vs_qs_comparison/dyn_high_force_high_clutter_small_test',
    ...
    qs_skin_nm_list = [
                        ...
                        'final_ijrr_results_with_10_second_timeout/75_obstacle_configurations_movable000_fixed080_25N_thresh/mpc_qs_1_using_skin',
                        #'dyn_vs_qs_comparison/qs_low_force_high_clutter',
...
def fixed_50_percent():
    ...
    pp.plot(obj_count_list, qs_skin_success_percent, 'b-.s', mew=0, label='MPC with Quasi-static Model')
    pp.plot(obj_count_list, dyn_skin_success_percent, 'g:D', mew=0, label='MPC with Dynamic Model')
...
def plot_perc_contact_forces_above_force_threshold(perc_dyn, perc_dyn_v1, perc_dyn_v2, perc_qs, x):
    ...
    # pp.plot(x, perc_dyn_v1, 'r--', label='MPC dyn v1')
    # pp.plot(x, perc_dyn_v2, 'k:', label='MPC dyn v2')
    #pp.xlim(5.5, 20.5)
...
def get_percentages_above_thresh(direc, nm, x_values):
    ...
    dyn_total_v1 = 0
    dyn_above_v1 = [0]*len(x_values)

    dyn_total_v2 = 0
    dyn_above_v2 = [0]*len(x_values)
    ...
    for name in nm:
        ...
        # dyn_data_v2 = ut.load_pickle(direc+'dyn_vs_qs_comparison/dyn_high_force_high_clutter_20130620_large_delta_t_impulse_0.02/reach_in_force_statistics.pkl')
        # dyn_data_v1 = ut.load_pickle(direc+'dyn_vs_qs_comparison/dyn_high_force_high_clutter_20130624_large_delta_t_impulse_0.02/reach_in_force_statistics.pkl')
        # #dyn_data =ut.load_pickle(direc+'dyn_vs_qs_comparison/dyn_high_force_high_clutter_simple_large_delta_t_impulse_0.02/reach_in_force_statistics.pkl')
        dyn_data =ut.load_pickle(direc+'dyn_vs_qs_comparison/dyn_high_force_high_clutter_new_delta_t_impulse_sim_anneal_search_set2_mu_15/reach_in_force_statistics.pkl')

                         
        #dyn_data = ut.load_pickle(direc+'dyn_vs_qs_comparison/dyn_high_force_high_clutter_simple_large_delta_t_impulse_0.02/reach_in_force_statistics.pkl')
        #dyn_data = ut.load_pickle(direc+'dyn_vs_qs_comparison/dyn_high_force_high_clutter_simple_large .../reach_in_force_statistics.pkl')
        ...
        qs_data = ut.load_pickle(direc+'final_ijrr_results_with_10_second_timeout/75_obstacle_configurations_movable000_fixed080_25N_thresh/mpc_qs_1_using_skin/reach_in_force_statistics.pkl')
        ...
        # dyn_total_v1 = dyn_total_v1 + len(dyn_data_v1['all_forces_mag_list'])
        # dyn_total_v2 = dyn_total_v2 + len(dyn_data_v2['all_forces_mag_list'])
        ...
        for i in xrange(len(x_values)):
            ...
            # dyn_above_v1[i] = dyn_above_v1[i] + len(np.where(np.array(dyn_data_v1['all_forces_mag_list']) > x_values[i])[0])
            # dyn_above_v2[i] = dyn_above_v2[i] + len(np.where(np.array(dyn_data_v2['all_forces_mag_list']) > x_values[i])[0])
    ...
    for i in xrange(len(x_values)):
        ...
        # dyn_result_v1.append(float(dyn_above_v1[i])/float(dyn_total_v1)*100)
        # dyn_result_v2.append(float(dyn_above_v2[i])/float(dyn_total_v2)*100)
        qs_result.append(float(qs_above[i])/float(qs_total)*100)

    #return dyn_result, dyn_total,  dyn_result_v1, dyn_total_v1,  dyn_result_v2, dyn_total_v2, qs_result, qs_total
    return dyn_result, dyn_total,  None, None, None, None, qs_result, qs_total
...
if __name__ == '__main__':
    ...
    if plot_percent_above_threshold:
        ...
        perc_dyn_50, dyn_total_50, perc_dyn_50_v1, dyn_total_50_v1, perc_dyn_50_v2, dyn_total_50_v2,  perc_qs_50, qs_total_50 = get_percentages_above_thresh(direc, nm_50, x_values)
        x_values2 = [25*1.1]
        ...
        d = {}
        d['high_high_qs'] = perc_qs_50
        d['high_high_dyn'] = perc_dyn_50
        d['high_high_x'] = x_values

        import scipy.io as io
        io.savemat('/home/mkillpack/Dropbox/high_high_force.mat', d)
        plot_perc_contact_forces_above_force_threshold(perc_dyn_50, perc_dyn_50_v1, perc_dyn_50_v2, perc_qs_50, x_values)
        pp.title('80 fixed obstacles \n (25N force threshold)')
        pp.ylim((0, 20.))
        pp.xlim((24, 37.))
        #pp.savefig('contact_force_stats_high_force_high_clutter_delta_t_impulse_0.04.pdf')
    ...
    if success_rate_plots:
        ...
        pp.savefig('success_rate_high_force_high_clutter_delta_t_impulse_0.04.pdf')
