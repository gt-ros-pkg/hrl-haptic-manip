...
#######################################################
# 
# cRoNA2 Move and Scoop Script
# 
########################################################
# script that commands robot joint angles and base to scoop body 
# requires the body position with respect to the robot
...
class cRoNA2MoveAndScoop(object):
    def __init__(self):
	...
	self.ragdoll_wrench_pub = rospy.Publisher("/gazebo/ragdoll_wrench_val", Float64MultiArray)
	self.actuated_rollers_pub = rospy.Publisher('/gazebo/actuated_rollers',Bool)
    ...
    def move_and_scoop(self,theta,object_x,object_y):
	...
	#force_z = 30
	#force_x = -45
	#world_force_x = math.cos(theta)*force_x-math.sin(theta)*force_y
	#world_force_y = math.sin(theta)*force_x+math.cos(theta)*force_y
	#fa.data += [world_force_x,world_force_y,force_z]
        self.ragdoll_wrench_pub.publish(fa)
	#for i in range(5):
	#    self.actuated_rollers_pub.publish(1)
	######################
	#offset_x = -0.9
        #offset_y = 0.25
	### Offset for new ragdoll
        ...
        self.base_goal = [object_x+world_offset_x,object_y+world_offset_y,self.base_ja[2]]
	#self.base_goal = [self.base_ja[0]+self.base_goal[0],self.base_ja[1]+self.base_goal[1],np.degrees(theta)]
        #self.base_goal = [4.1,0.075,0.]
        self.set_pose = self.base_goal+self.torso_ja+self.head_ja+self.larm_ja+self.rarm_ja
	#for i in range(5):
        #    self.actuated_rollers_pub.publish(1)
        #self.send_goal(self.set_pose,1,100)
	...
	self.torso_goal = [-90.]
        self.head_goal = [90.]
        self.larm_goal = [115.,0.,0.,75.,0.,45.]
        self.rarm_goal = [115.,0.,0.,75.,0.,45.]
        self.set_pose = self.base_ja+self.torso_goal+self.head_goal+self.larm_goal+self.rarm_goal
        self.send_goal(self.set_pose,5,10)
	#for i in range(5):
        #    self.actuated_rollers_pub.publish(0)
	...
	self.move_and_scoop(theta,object_x,object_y)
	for i in range(5):
            self.scripted_behaviors_publishing.publish(0)
	    self.base_control_pub.publish(0)

if __name__ == '__main__':
    rospy.init_node('crona2_move_and_scoop')
    c2mas = cRoNA2MoveAndScoop()
    c2mas.start()
