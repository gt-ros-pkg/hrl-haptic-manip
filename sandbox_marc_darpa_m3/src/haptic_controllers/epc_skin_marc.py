...
class Skin_EPC_marc(es.Skin_EPC):
    ...
    def teleop_greedy_to_goal(self, goal_pos, goal_orientation, control_param_dict,
                              eq_gen_type, logging_name):
    ...
    def greedy_to_goal_marc(self, goal_pos, goal_orientation,
                       threshold_dict, timeout,
                       eq_gen_type,
                       fragile_center = None, fragile_radius = None):
        threshold_dict['goal_pos'] = goal_pos
        threshold_dict['goal_orientation'] = goal_orientation
    ...
    def greedy_pull_in_and_out(self, goal_pos, goal_orientation, eq_gen_type, q_home, gtg_dict):
        ...
        res = self.greedy_to_goal_marc(goal_pos, goal_orientation, 
                                       gtg_dict, 40., eq_gen_type)
        ...
        for i in xrange(len(goals)):
            res_interm = self.greedy_to_goal_marc(np.matrix(goals[i]).T, goal_orientation,
                                                  gtg_dict, 20., eq_gen_type)
            print "Way Point result is :", res_interm

            res = self.greedy_to_goal_marc(goal_pos, goal_orientation,
                                           gtg_dict, 30., eq_gen_type)
    ...
    def torque_vmc_set_pt_gen(self, ep_gen):
        ...
        kp_joints, kd_joints = self.robot.get_joint_impedance()
        K = np.diag(kp_joints)
        K_d = np.diag(kd_joints)
...
########################################################################################
        ...
        # self.max_jtlim_arr = np.radians(np.array([150., 162, 159.]))
            ...
            q_eq = K_inv*(tau_cmd + K*np.matrix(q).reshape(self.robot.kinematics.n_jts,1) 
                          + K_d*np.matrix(self.robot.qdot).reshape(self.robot.kinematics.n_jts,1))
...
####################################################################################################
#################this is old code that is now implemented in the nominal epc_skin.py################
####################################################################################################
#     def populate_request(self, delta_x_g, J_h, K_j, Kc_l, Jc_l, n_l, q, f_n, max_force_mag, delta_f_min, delta_f_max, Q):
...
#         x_g = ep_gen.goal_pos
#         x_orientation = ep_gen.goal_orientation
...
#         max_delta_force_mag = t * ep_gen.max_delta_force_mag
...
#         # This weights the importance of acheiving the goal states
#         # relative to one another (i.e. is position more important 
#         # than orientation?)
#         goal_state_weights = ep_gen.goal_state_weights


#         task_ind = ep_gen.task_index
...
#             ###########
#             #ADD STOPPING CRITERIA FOR ORIENTATION
#             ###########
...
#             #this is temporary, need to make my own function to define where 
#             #desired orientation comes from
            
#             err_theta = 3.14/2.0-(q[0]+q[1]+q[2])
            
#             if abs(err_theta) > 0.001 :
#                 des_d_theta = np.sign(err_theta)*0.001
#             else:
#                 des_d_theta = err_theta

#             delta_x_g = (delta_x_g[0:2]).A1.tolist()

#             #delta_x_g = [delta_x_g[0,0], delta_x_g[1,0], des_d_theta]
#             delta_x_g.append(des_d_theta)
...
#             req = self.populate_request(delta_x_g, J_h, K_j, Kc_l, Jc_l, n_l, q, f_n, max_force_mag, delta_f_min, delta_f_max, Q)
            
#             try:
#                 #this is the service call to the mpc optimization code (c++)
#                 res = self.get_delta_phi_opt(req)
#                 jep[0:m] = (phi_curr + np.matrix(res.delta_phi_opt).reshape(m,1)).A1
