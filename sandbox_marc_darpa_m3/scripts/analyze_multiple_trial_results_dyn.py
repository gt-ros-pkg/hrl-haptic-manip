...
class ComputeResultStatistics():
    def __init__(self, robot= None, contact_vel_save = True):
        self.root_path = None
        if robot == None:
            self.robot = sim_robot.ODESimArm(sim_robot_config)
        else:
            self.robot = robot
        self.contact_vel_save = contact_vel_save
        ...
        for root, dirs, files in os.walk(root_path):
            ...
            for d in dirs:
                ...
                for fn in fn_list:
                    ...
                    #     fn(full_path)
                    # except:
                    #     print full_path, " is still running on a machine"
                    #     break
    ...
    def get_sum_taxel_hist(self, full_path):
        ...
        res_dict = ut.load_pickle(full_path+'/reach_log.pkl')
    ...
    def get_contact_loc_hist(self, full_path):
        if full_path == 'start':
            ...
            # self.vel_hist_above_5N = None
            # self.vel_hist_above_10N = None
            # self.vel_hist_above_25N = None
            # self.vel_hist_above_40N = None
        ...
        if full_path == 'end':
            ...
            # d['multi_contact_vel_hist_above_5N'] = self.vel_hist_above_5N
            # d['multi_contact_vel_hist_above_10N'] = self.vel_hist_above_10N
            # d['multi_contact_vel_hist_above_25N'] = self.vel_hist_above_25N
            # d['multi_contact_vel_hist_above_40N'] = self.vel_hist_above_40N
        ...
        if self.multi_contact_hist == None:
            try:
                # self.vel_hist_above_5N = []
                # self.vel_hist_above_10N = []
                # self.vel_hist_above_25N = []
                # self.vel_hist_above_40N = []
        ...
        if res_dict is None:
            ...
            try:
                ...
                qd_s = np.array(res_dict['qdot_list'])[num_contacts_ind]
                ...
                for j in xrange(len(num_contacts)):
                    ...
                    for i in xrange(int(num_contacts[j])):
                        ...
                        # if num_contacts[j] > 1:
                        #     cont_x.append(cont_loc[0,0])
                        #     cont_y.append(cont_loc[1,0])
                        ...
                        # jac = self.robot.kinematics.jacobian(q_s[j], np.matrix(res_dict['all_forces_locs_list'][ind]).reshape(3,1))
                        # jac = jac[0:2, 0:res_dict['all_forces_jts_list'][ind]+1]
                        # vel = jac*(np.matrix(qd_s[j]).reshape(3,1))[0:res_dict['all_forces_jts_list'][ind]+1,:]
                    ...
                    #     pl.plot([jt_pos[0,0]], [jt_pos[1,0]], 'ko')
                    #     pl.hold(True)
                    #     print "cont x and y :", cont_x, cont_y
                    #     pl.plot(cont_x, cont_y, 'r*')
                    #     pl.show()
            except:
    ...
    def get_impact_and_jamming_data(self, full_path):
        ...
        if full_path == 'end':
            ...
            if self.contact_vel_save:
                ut.save_pickle(d, self.root_path + '/stats_on_impact_velocity.pkl')
            else:
                self.contact_data = d

        full_path = full_path.rstrip('/')
        data = ut.load_pickle(full_path+'/reach_log.pkl')
        trial = full_path.split('/')[-1]
    ...
    def trials_where_mean_stopped_moving(self, full_path):
        ...
        full_path = full_path.rstrip('/')
        log_dict = ut.load_pickle(full_path+'/reach_log.pkl')
    ...
    def success_rate_for_different_stopping_force(self, full_path):
        ...
        pkl_list = ut.get_bash_command_output('ls -t '+full_path+'/reach_log.pkl')
        pkl_list.reverse()
