...
class ode_SkinClient(SkinClient):
    ...
    # Return contact point locations based on ? coordinate
    def force_contact_point_list(self, normal_component_only,
                                    return_time=False):
        f_l, n_l, pts_l, stamp = self.get_pts_snapshot()

        if return_time:
            return f_l, n_l, pts_l, stamp
        else:
            return f_l, n_l, pts_l
